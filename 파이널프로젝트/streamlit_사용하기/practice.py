...
# 한글 폰트 설정
plt.rcParams['font.family'] = 'Malgun Gothic'  # 한글 폰트 이름 설정
##########################################################################################
import folium
import requests
import json
...
st.subheader("지역 특성 비교")

# 지역 목록
places = [
    "명동", "홍대", "강남역", "이태원", "신촌", "여의도", "북촌", "성수동", 
    "청담동", "연남동", "망원동", "을지로", "성내동", "문래동", "봉천동", 
    "녹사평", "도곡동", "불광동", "장위동", "수색동"
]
...
# 지역1과 지역2 선택
region1 = st.selectbox("지역1을 선택하세요:", places)
region2 = st.selectbox("지역2를 선택하세요:", places)

# 임의의 데이터 생성 (예시)
np.random.seed(0)
data = {
    place: np.random.rand(5) * 10 for place in places
}
