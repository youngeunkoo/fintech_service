...
with col1:
    # 지도 표시
    selected_data = locations[selected_location]
    m = folium.Map(location=selected_data["coords"], tiles="Cartodb Positron", zoom_start=15)

    # 폴리곤 추가
    folium.Polygon(
        locations=selected_data["polygon"],
        color="blue",
        fill=True,
        fill_color="blue",
        fill_opacity=0.3
...
with col2:
    # 오른쪽 정보 표시
    #st.subheader(f"{selected_location} 지역 정보")
    st.title(f"{selected_location}")

    
    # 분위기 키워드
    st.markdown("### 분위기 키워드")
    keywords = selected_data["keywords"]
    st.markdown(
        f"""
        <style>
        .keyword {{
    ...
    # 이미지 출력
    st.markdown("### 이미지 보기")
    st.image(selected_data['image'], caption=f"{selected_location} 이미지", use_column_width=True)
